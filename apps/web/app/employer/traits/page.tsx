...
import type { MatchedCandidate, TraitCandidate, TraitStat } from "@/lib/types";
...
function LiveTraits({ cands }: { cands: MatchedCandidate[] }) {
  // Only candidates who've completed the (compulsory) quiz have a trait —
  // older matches predating that requirement can still be null.
  const profiled = cands.filter((c): c is MatchedCandidate & { trait: NonNullable<MatchedCandidate["trait"]> } => !!c.trait);
  const total = profiled.length;

  const counts = new Map<string, number>();
  for (const c of profiled) counts.set(c.trait, (counts.get(c.trait) ?? 0) + 1);
  const liveTraitStats: TraitStat[] = [...counts.entries()]
    .sort((a, b) => b[1] - a[1])
    .map(([trait, count]) => ({
  ...
  const liveCandidates: TraitCandidate[] = profiled.map((c) => {
