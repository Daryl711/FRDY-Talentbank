...
export default function AnimalTraitsView({
  subjectPlural = "candidates",
  ...
  return (
    <>
      <PageHeader
        title="Animal Traits"
        subtitle={`Personality archetypes across your ${subjectPlural} — ${traitsProfiled} profiled`}
        action={
          <button
            onClick={() => generateTraitsReport(traitStats, traitCandidates, subjectPlural)}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        {traitStats.map((t) => (
          <Panel key={t.trait} className="p-5 text-center">
            ...
            <div className="eyebrow mt-2">{t.count} {subjectPlural}</div>
          </Panel>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
        <Panel className="p-6">
          ...
          <p className="text-mut text-[12px] mt-1">All {subjectPlural} · active profiles</p>
        ...
        <div className="flex flex-col gap-6">
          <Panel className="p-6">
            <h2 className="font-serif text-[22px] font-bold text-ink mb-4">{listTitle}</h2>
