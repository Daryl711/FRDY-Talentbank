...
import { getUniversityCandidates, uniName, type UniversityCandidate } from "@/lib/university";
...
export default function AnimalTraitsPage() {
  ...
  if (isSupabaseConfigured && candidates === null) {
    return (
      <>
        <PageHeader title="Animal Traits" subtitle="Loading your students…" />
  ...
  if (candidates && candidates.length > 0) {
    return <LiveTraits candidates={candidates} />;
  }
...
/* ================================================================ LIVE TRAITS */
function LiveTraits({ candidates }: { candidates: UniversityCandidate[] }) {
  const { stats: liveTraitStats, candidates: liveCandidates, total } = computeTraitBreakdown(
    candidates.map((c) => ({
      id: c.id,
      name: c.name,
      initials: c.initials,
      role: c.headline ?? "Student",
      trait: c.animalTrait,
      match: c.confidence ?? 0,
  ...
  return (
    <>
      <PageHeader
        title="Animal Traits"
        subtitle={`Personality archetypes across your students at ${uniName} — ${total} profiled`}
        action={
          <button
            onClick={() => generateTraitsReport(liveTraitStats, liveCandidates, "students")}
            disabled={total === 0}
            title={total === 0 ? "No profiled students yet to report on" : "Download an animal traits report (.xlsx)"}
      ...
      {total === 0 ? (
        <Panel className="p-16 text-center text-mut text-[13px] mt-6">
          No students have completed the Animal Persona quiz yet.
        </Panel>
      ) : (
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
            {liveTraitStats.map((t) => (
              <Panel key={t.trait} className="p-5 text-center">
                ...
                <div className="eyebrow mt-2">{t.count} students</div>
              </Panel>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
            ...
            <div className="flex flex-col gap-6">
              <Panel className="p-6">
                <h2 className="font-serif text-[22px] font-bold text-ink mb-4">All Students</h2>
...
function MockTraits() {
  ...
  return (
    <>
      <PageHeader
        title="Animal Traits"
        subtitle={`Personality archetypes across your students — ${traitsProfiled} profiled`}
        action={
          <button
            onClick={() => generateTraitsReport(traitStats, traitCandidates, "students")}
