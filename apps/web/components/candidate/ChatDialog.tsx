...
export default function ChatDialog({
  ...
  // The dialog remounts each time it's opened (rendered only while a peer is
  // set), so initializing from connectionId is correct — no synchronous
  // setState needed inside the effect below.
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.6)" }} onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-line">
          <div className="relative w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-[14px] shrink-0" style={{ backgroundColor: peer.color }}>
            {peer.initials}
            {peer.online && <span className="absolute -bottom-[2px] -right-[2px] w-3 h-3 rounded-full bg-ok border-2 border-bg" />}
          </div>
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-[15.5px] text-ink truncate">{peer.name}</div>
            <div className="text-dim text-[12.5px] truncate">{peer.role}</div>
          </div>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 flex flex-col gap-[10px]">
          {loading ? (
            <div className="flex-1 flex items-center justify-center text-mut text-[13px]">Loading…</div>
          ) : messages.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center gap-3 text-dim">
              ...
              <p className="text-[13.5px]">
                Say hello to {peer.name.split(" ")[0]}.<br />This is the start of your conversation.
              </p>
            </div>
          ) : (
            messages.map((m) => (
